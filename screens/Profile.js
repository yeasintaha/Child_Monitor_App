...
import {View, StyleSheet, TouchableOpacity, SafeAreaView, KeyboardAvoidingView, Text, TextInput, ScrollView, Dimensions} from 'react-native';
...
const Profile = ({ navigation: { goBack } }) => {
    ...
    const handleProfileUpdate = ()=>{
        ...
        if((!phone.includes("01", 0) || (phone?.length !=11) || (!(/^\d+$/.test(phone)))) && flag!=1){
            flag=1;
        ...
        if (flag==0){
            let userDoc = doc(db_firestore, "User_Data", (userCredentials.Username + userCredentials.Device)); 

            const userData = {
                "Username": userCredentials.Username, 
                "Email":userCredentials.Email, 
                "Device": userCredentials.Device, 
    ...
    return (
        <SafeAreaView style={styles.profileContainer}>
            ...
            <TouchableOpacity 
                style={{flex:0, flexDirection:"row", padding:5, marginLeft:10, marginTop:30, justifyContent:"flex-start", zIndex:1}}
                onPress={() => goBack()}>
                <Icon name= {'chevron-back-sharp'} size={40} color={'rgba(0, 0, 255, 0.7)'} /> 
            </TouchableOpacity>
            <View style={styles.profileHeader}>
                <FontAwesome name={'user-circle'} size={40} color={'rgba(0, 0, 255, 0.7)'}  style={styles.profileIcon}/>
            ...
            <View style={styles.componentView}>
                <Text style={styles.componentViewHeader}>Device ID :</Text>
                <Text style={styles.componentViewText}>{userCredentials.Device}</Text>
            </View>
            <KeyboardAvoidingView style={styles.componentView}>
