import { ActivityIndicator,ScrollView, ToastAndroid, KeyboardAvoidingView, View, Text, Linking, ImageBackground, Dimensions, StyleSheet, Image, TextInput, Platform, Keyboard} from 'react-native'
...
const Login = ({navigation}) => {
  ...
  const { emailProvider, setEmailProvider, setUserCredentials } = useContext(UserContext);
    ...
    if(flag==0){
      ...
      .catch((re)=>{
        Toast.show('Email or Password didn\'t match!', {
            ...
            position: Toast.positions.BOTTOM,
            containerStyle:{backgroundColor:"red",borderRadius:25, padding:10}
          }); 
        setEmailRedBox(true); 
        setPasswordRedBox(true); 
  ...
  return (
    <>
        <ImageBackground 
            ...
            >
              <Icon name={showpass==false ? 'ios-eye-outline' : 'ios-eye-off-outline'} size={24} color={'black'} onPress ={()=>setShowpass(!showpass)} />
            </TouchableOpacity>
              ...
              <KeyboardAvoidingView behavior='padding'>
                <TouchableOpacity style = {[styles.btnLogin, {backgroundColor:"rgba(255, 255, 255, 0.5)", borderColor:"dodgerblue", borderRadius:10, borderWidth:3}]} onPress={()=>navigation.navigate('Register')}>
                  <Text style = {{color:"dodgerblue", fontSize:16, fontWeight:"bold"}}> Register</Text>
                </TouchableOpacity>
              </KeyboardAvoidingView>
              <View style={{width:"100%", height:200}}></View>
            </Animatable.View>
        </ImageBackground>
        { loginPending  && platformAndroid  ? <CircleLoader/>  : 
          loginPending && !platformAndroid ? 
          <View style={{top:0, position:"absolute", justifyContent:"center",zIndex:100, width:"100%", height:"100%", backgroundColor:"lightblue"}}>
